...
class MockFile {
  ...
  async arrayBuffer(): Promise<ArrayBuffer> {
    ...
    if (this.type === "application/pdf") {
      ...
      const repeatContent = textContent.repeat(
        Math.ceil(this.size / textContent.length)
      )
      for (let i = 0; i < this.size; i++) {
...
describe("FileValidationService", () => {
  ...
  describe("getFileExtension", () => {
    it("should extract file extensions correctly", () => {
      expect(FileValidationService.getFileExtension("document.pdf")).toBe(
        ".pdf"
      )
    ...
    it("should handle multiple dots in filename", () => {
      expect(FileValidationService.getFileExtension("my.document.pdf")).toBe(
        ".pdf"
      )
    })
  })
  ...
  describe("formatFileSize", () => {
    it("should format file sizes correctly", () => {
      ...
      expect(FileValidationService.formatFileSize(1024 * 1024 * 1024)).toBe(
        "1 GB"
      )
    })
  })
  ...
  describe("getMimeTypeFromExtension", () => {
    it("should return correct MIME types for extensions", () => {
      expect(FileValidationService.getMimeTypeFromExtension(".pdf")).toBe(
        "application/pdf"
      )
      expect(FileValidationService.getMimeTypeFromExtension(".epub")).toBe(
        "application/epub+zip"
      )
      expect(FileValidationService.getMimeTypeFromExtension(".docx")).toBe(
        "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
      )
      expect(FileValidationService.getMimeTypeFromExtension(".txt")).toBe(
        "text/plain"
      )
    })
    ...
    it("should return null for unsupported extensions", () => {
      ...
      expect(
        FileValidationService.getMimeTypeFromExtension(".unknown")
      ).toBeNull()
    })
  })
})
