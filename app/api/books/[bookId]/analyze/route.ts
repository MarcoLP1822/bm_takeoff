...
import { compressBookContent, decompressBookContent } from '@/lib/compression-service'
...
export async function POST(
  ...
  try {
    ...
    const book = await RetryHandler.withRetry(async () => {
    ...
    if (!book.textContent) {
      return NextResponse.json(
        { error: 'Book has no text content to analyze. Please re-upload the file or try text extraction again.' },
        { status: 400 }
      )
    }
    ...
    processBookAnalysis(bookId, preparedText, book.title, book.author, userId)
      .catch(error => {
...
async function processBookAnalysis(
  ...
  try {
    ...
    const analysisResult = await RetryHandler.withRetry(async () => {
