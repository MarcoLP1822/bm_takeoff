...
describe('HashtagGenerator', () => {
  ...
  describe('generateHashtags', () => {
    test('should generate hashtags for Twitter', () => {
      ...
      const platformHashtags = suggestions.filter(s => s.category === 'platform')
    ...
    test('should include genre-specific hashtags', () => {
      ...
      const businessHashtags = suggestions.filter(s => 
    ...
    test('should vary hashtags by content type', () => {
      ...
      const intersection = quoteHashtags.filter(h => insightHashtags.includes(h))
      // Should have some different hashtags, but allow for some overlap
      expect(intersection.length).toBeLessThanOrEqual(Math.min(quoteHashtags.length, insightHashtags.length))
    })
  ...
  describe('getOptimalHashtags', () => {
    test('should return optimal hashtag combination', () => {
      ...
      const optimal = HashtagGenerator.getOptimalHashtags(suggestions, 'twitter')
      ...
      const originalSuggestions = suggestions.filter(s => optimal.includes(s.hashtag))
    ...
    test('should respect platform limits', () => {
      ...
      const twitterOptimal = HashtagGenerator.getOptimalHashtags(suggestions, 'twitter')
      const instagramOptimal = HashtagGenerator.getOptimalHashtags(suggestions, 'instagram')
    ...
    test('should balance different categories', () => {
      ...
      const optimal = HashtagGenerator.getOptimalHashtags(suggestions, 'instagram')
      ...
      expect(Math.abs(bookHashtags.length - platformHashtags.length)).toBeLessThan(optimal.length)
    })
  })
  ...
  describe('analyzeHashtagPerformance', () => {
    test('should analyze hashtag performance for Twitter', () => {
      ...
      const analysis = HashtagGenerator.analyzeHashtagPerformance(hashtags, 'twitter')
    ...
    test('should reward platform-specific hashtags', () => {
      ...
      const withAnalysis = HashtagGenerator.analyzeHashtagPerformance(withPlatformHashtags, 'twitter')
      const withoutAnalysis = HashtagGenerator.analyzeHashtagPerformance(withoutPlatformHashtags, 'twitter')
    ...
    test('should suggest improvements', () => {
      ...
      const analysis = HashtagGenerator.analyzeHashtagPerformance(poorHashtags, 'instagram')

      expect(analysis.suggestions.length).toBeGreaterThan(0)
      expect(analysis.feedback.some(f => f.includes('more hashtags'))).toBe(true)
    })

    test('should recognize good hashtag usage', () => {
      const goodHashtags = ['#bookstagram', '#reading', '#booklovers', '#inspiration', '#bookquote']
      const analysis = HashtagGenerator.analyzeHashtagPerformance(goodHashtags, 'instagram')

      expect(analysis.score).toBeGreaterThan(60)
      expect(analysis.feedback.some(f => f.includes('Good') || f.includes('Great'))).toBe(true)
    })

    test('should handle different platforms appropriately', () => {
      const hashtags = ['#BookQuote', '#Reading', '#Inspiration']
      
      const twitterAnalysis = HashtagGenerator.analyzeHashtagPerformance(hashtags, 'twitter')
      const instagramAnalysis = HashtagGenerator.analyzeHashtagPerformance(hashtags, 'instagram')
      const linkedinAnalysis = HashtagGenerator.analyzeHashtagPerformance(hashtags, 'linkedin')
      ...
      if (instagramAnalysis.suggestions.length > 0) {
        const hasHashtagSuggestion = instagramAnalysis.suggestions.some(s => 
          s.toLowerCase().includes('hashtag') || s.toLowerCase().includes('more')
        )
