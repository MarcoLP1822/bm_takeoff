...
describe("TextExtractionService", () => {
  describe("isValidFileFormat", () => {
    it("should return true for supported MIME types", () => {
      expect(TextExtractionService.isValidFileFormat("application/pdf")).toBe(
        true
      )
      expect(
        TextExtractionService.isValidFileFormat("application/epub+zip")
      ).toBe(true)
      expect(
        TextExtractionService.isValidFileFormat(
    ...
    it("should return false for unsupported MIME types", () => {
      ...
      expect(TextExtractionService.isValidFileFormat("application/json")).toBe(
        false
      )
  ...
  describe("getFileExtension", () => {
    it("should return correct extensions for MIME types", () => {
      expect(TextExtractionService.getFileExtension("application/pdf")).toBe(
        ".pdf"
      )
      expect(
        TextExtractionService.getFileExtension("application/epub+zip")
      ).toBe(".epub")
      expect(
        TextExtractionService.getFileExtension(
          "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        )
      ).toBe(".docx")
