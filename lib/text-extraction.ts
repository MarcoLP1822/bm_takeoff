...
export class TextExtractionService {
   ...
   */
  private static async extractFromPDF(buffer: Buffer, fileName?: string): Promise<TextExtractionResult> {
    try {
      ...
      const title = fileName ? fileName.replace(/\.[^/.]+$/, '') : (data.info?.Title || "PDF Document")
      
      return {
        text: data.text || 'No text content found in PDF',
        metadata: {
          pages: data.numpages,
          wordCount: data.text ? data.text.split(/\s+/).filter((word: string) => word.length > 0).length : 0,
          title,
      ...
      const fallbackTitle = fileName ? fileName.replace(/\.[^/.]+$/, '') + " (Failed)" : "PDF Document (Failed)"
      return {
   ...
   */
  private static async extractFromEPUB(buffer: Buffer, fileName?: string): Promise<TextExtractionResult> {
    try {
      ...
      for (const entry of entries) {
        ...
        if (!fileName && (entry.entryName.includes('content.opf') || entry.entryName.includes('metadata'))) {
          const metaContent = entry.getData().toString('utf8')
          const titleMatch = metaContent.match(/<dc:title[^>]*>(.*?)<\/dc:title>/i)
      ...
      const fallbackTitle = fileName ? fileName.replace(/\.[^/.]+$/, '') + " (Failed)" : "EPUB Document (Failed)"
      return {
   ...
   */
  private static async extractFromDOCX(buffer: Buffer, fileName?: string): Promise<TextExtractionResult> {
    try {
      ...
      const title = fileName ? fileName.replace(/\.[^/.]+$/, '') : "DOCX Document"
      ...
      const fallbackTitle = fileName ? fileName.replace(/\.[^/.]+$/, '') + " (Failed)" : "DOCX Document (Failed)"
      return {
   ...
   */
  private static async extractFromTXT(buffer: Buffer, fileName?: string): Promise<TextExtractionResult> {
