...
describe('ErrorCreators', () => {
  ...
  describe('external service errors', () => {
    ...
    it('should create social media API error', () => {
      const error = ErrorCreators.socialMediaApiError('Twitter', { code: 'RATE_LIMIT' })
  ...
  describe('content management errors', () => {
    it('should create content generation failed error', () => {
      const error = ErrorCreators.contentGenerationFailed('AI quota exceeded', { service: 'OpenAI' })
      
      expect(error.type).toBe(ErrorType.EXTERNAL_SERVICE)
      expect(error.statusCode).toBe(422)
      expect(error.retryable).toBe(true)
      expect(error.details).toEqual({ reason: 'AI quota exceeded', service: 'OpenAI' })
    })
    ...
    it('should create book analysis required error', () => {
      ...
      expect(error.type).toBe(ErrorType.VALIDATION)
      expect(error.statusCode).toBe(400)
      expect(error.userMessage).toBe('"Test Book" must be analyzed before generating content. Please run the analysis first.')
    })
  })
})
...
describe('ValidationHelpers', () => {
  ...
  describe('email validation', () => {
    it('should pass for valid emails', () => {
      ...
      expect(() => ValidationHelpers.email('user.name+tag@domain.co.uk')).not.toThrow()
    })
  ...
  describe('length validations', () => {
    it('should validate minimum length', () => {
      expect(() => ValidationHelpers.minLength('test', 3, 'field')).not.toThrow()
      expect(() => ValidationHelpers.minLength('ab', 3, 'field')).toThrow()
    })

    it('should validate maximum length', () => {
      expect(() => ValidationHelpers.maxLength('test', 5, 'field')).not.toThrow()
  ...
  describe('date validations', () => {
    it('should validate dates', () => {
      expect(() => ValidationHelpers.isValidDate('2023-01-01', 'field')).not.toThrow()
      expect(() => ValidationHelpers.isValidDate('invalid-date', 'field')).toThrow()
    })
    ...
    it('should validate future dates', () => {
      ...
      expect(() => ValidationHelpers.futureDate(futureDate, 'field')).not.toThrow()
