...
describe("ImageSuggestionEngine", () => {
  ...
  describe("generateImageSuggestions", () => {
    ...
    test("should generate theme image suggestions", () => {
      ...
      expect(
        types.includes("concept-art") || types.includes("quote-graphic")
      ).toBe(true)
    })
    ...
    test("should generate summary image suggestions", () => {
      ...
      const types = suggestions.map(s => s.type)
      expect(types.includes("book-cover") || types.includes("collage")).toBe(
        true
      )
    })
    ...
    test("should adapt dimensions for different platforms", () => {
      ...
      const instagramSuggestions =
        ImageSuggestionEngine.generateImageSuggestions(
          "Test content",
          "instagram",
          "quote",
          "Test Book"
    ...
    test("should include appropriate elements for each suggestion type", () => {
      ...
      expect(
        quoteElements.some(e => e.includes("typography") || e.includes("quote"))
      ).toBe(true)
    })
    ...
    test("should vary styles based on genre", () => {
      ...
      expect(
        businessStyles.includes("professional") ||
          businessStyles.includes("modern")
      ).toBe(true)

      // Fiction should allow more artistic styles
      const fictionStyles = fictionSuggestions.map(s => s.style)
      expect(
        fictionStyles.includes("artistic") || fictionStyles.includes("vintage")
      ).toBe(true)
    })
  })
  ...
  describe("generateImageUrl", () => {
    test("should generate valid image URL", () => {
      ...
      const url = ImageSuggestionEngine.generateImageUrl(
        suggestion,
        "Test content"
      )
  ...
  describe("color and style selection", () => {
    test("should select appropriate colors for themes", () => {
      ...
      expect(
        leadershipColors.some(
          c => c.includes("#1f4e79") || c.includes("#2c5aa0")
        )
      ).toBe(true)
      ...
      expect(
        loveColors.some(c => c.includes("#e74c3c") || c.includes("#ff6b9d"))
      ).toBe(true)
    })
