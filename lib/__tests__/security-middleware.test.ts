...
describe("Security Middleware", () => {
  ...
  describe("createSecurityMiddleware", () => {
    ...
    it("should handle rate limit exceeded", async () => {
      ...
      jest.doMock("../rate-limiting", () => ({
        rateLimiters: { api: {} },
        createRateLimitMiddleware: jest
          .fn()
          .mockReturnValue(mockRateLimitMiddleware)
      }))
  ...
  describe("createInputValidationMiddleware", () => {
    it("should validate input successfully", async () => {
      ...
      expect(
        (req as NextRequest & { validatedBody: unknown }).validatedBody
      ).toEqual({
        title: "Valid Title",
        content: "Valid content"
      })
    ...
    it("should fail validation for invalid input", async () => {
      ...
      expect(responseBody.details).toContain(
        "title: Must be no more than 10 characters long"
      )
    })
    ...
    it("should sanitize input data", async () => {
      ...
      expect(result).toBeNull()
      expect(
        (req as NextRequest & { validatedBody: { content: string } })
          .validatedBody.content
      ).toBe("Content withnull bytesand control chars")
    })
  })
