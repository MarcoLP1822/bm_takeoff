...
describe('BookDetail', () => {
  ...
  it('shows analysis results when completed', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('This is a test book summary.')).toBeInTheDocument()
  ...
  it('shows start analysis button for pending books', async () => {
    const pendingBook = { ...mockBookDetail, analysisStatus: 'pending' as const, analysisData: null }
    const mockResponse = {
      ok: true,
  ...
  it('shows processing state during analysis', async () => {
    const processingBook = { ...mockBookDetail, analysisStatus: 'processing' as const, analysisData: null }
    const mockResponse = {
      ok: true,
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('This may take a few minutes depending on the book length.')).toBeInTheDocument()
    })
  })

  it('shows retry button for failed analysis', async () => {
    const failedBook = { ...mockBookDetail, analysisStatus: 'failed' as const, analysisData: null }
    const mockResponse = {
      ok: true,
  ...
  it('starts analysis when button is clicked', async () => {
    ...
    await waitFor(() => {
      expect(fetch).toHaveBeenCalledWith('/api/books/1/analyze', { method: 'POST' })
    })
  })
