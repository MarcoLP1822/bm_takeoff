...
export function LazyContentManager({
  ...
  const loadContent = useCallback(async (offset: number, limit: number) => {
  ...
  const renderContentItem = useCallback((content: ContentItem) => {
    ...
    const variation: ContentVariation = {
      ...
      posts: [{
        ...
        isValid: content.content.length <= getPlatformMaxLength(content.platform),
        validationErrors: []
      }],
    ...
    return (
      <div key={content.id} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
        <ContentManager
          ...
          onSaveContentAction={onSaveContentAction || (() => Promise.resolve())}
          onDeleteVariationAction={onDeleteVariationAction || (() => Promise.resolve())}
  ...
  return (
    <div className={className}>
      ...
      <LazyLoadingList
        ...
        itemsPerPage={10}
        className="space-y-4"
        dependencies={[searchQuery, selectedPlatform, selectedStatus, selectedBook]}
      />
    </div>
  )
}
