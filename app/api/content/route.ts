...
import { z } from 'zod'
import { getOptimizedContentList, ContentListFilters } from '@/lib/database-optimization'
...
const querySchema = z.object({
  ...
  platform: z.enum(['twitter', 'instagram', 'linkedin', 'facebook']).nullable().optional(),
  status: z.enum(['draft', 'scheduled', 'published', 'failed']).nullable().optional(),
  contentType: z.enum(['post', 'story', 'article']).nullable().optional(),
  bookId: z.string().nullable().optional(),
  sortBy: z.enum(['createdAt', 'updatedAt', 'platform', 'status', 'bookTitle']).nullable().optional().default('createdAt'),
...
export async function GET(request: NextRequest) {
  try {
    ...
    const filters: ContentListFilters = {
      ...
      sortBy: query.sortBy === 'createdAt' ? 'created_at' : 
