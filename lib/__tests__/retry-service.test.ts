...
describe("RetryService", () => {
  ...
  describe("withRetry", () => {
    ...
    it("should not retry non-retryable errors", async () => {
      ...
      await expect(RetryService.withRetry(operation)).rejects.toThrow(
        nonRetryableError
      )
      expect(operation).toHaveBeenCalledTimes(1)
    })
    ...
    it("should respect maxAttempts", async () => {
      ...
      await expect(
        RetryService.withRetry(operation, {
          maxAttempts: 2,
          baseDelay: 10
        })
  ...
  describe("retry statistics", () => {
    it("should track active retries", async () => {
      ...
      const promise = RetryService.withRetry(
        slowOperation,
        {},
        {
