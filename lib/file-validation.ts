...
export class FileValidationService {
  ...
  static async validateFile(file: File): Promise<FileValidationResult> {
    ...
    const supportedMimeTypes = SUPPORTED_FILE_FORMATS.map(
    ...
    const expectedFormat = SUPPORTED_FILE_FORMATS.find(
  ...
  static isExtensionSupported(extension: string): boolean {
    return SUPPORTED_FILE_FORMATS.some(
  ...
  static getMimeTypeFromExtension(extension: string): string | null {
    const format = SUPPORTED_FILE_FORMATS.find(
      format => format.extension.toLowerCase() === extension.toLowerCase()
    )
  ...
  static async performSecurityScan(
    file: File
  ): Promise<{ passed: boolean; threats?: string[] }> {
    ...
    try {
      ...
      const headerThreats = this.checkFileHeader(
