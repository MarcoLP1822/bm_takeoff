...
describe('BookLibrary', () => {
  ...
  it('shows empty state when no books', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Upload your first book to get started with content generation')).toBeInTheDocument()
    })
  })
  ...
  it('filters books based on search query', async () => {
    ...
    const searchInput = screen.getByPlaceholderText('Search books by title, author, or genre...')
  ...
  it('handles book deletion API call', async () => {
    ...
    const bookLibraryInstance = screen.getByText('Test Book 1').closest('.border')
  ...
  it('handles fetch errors gracefully', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByPlaceholderText('Search books by title, author, or genre...')).toBeInTheDocument()
    }, { timeout: 3000 })
