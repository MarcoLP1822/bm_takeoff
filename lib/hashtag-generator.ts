...
export class HashtagGenerator {
   ...
   */
  private static generateBookHashtags(
    bookTitle: string,
    author?: string
  ): HashtagSuggestion[] {
    const suggestions: HashtagSuggestion[] = []
   ...
   */
  private static generatePlatformHashtags(
    platform: Platform
  ): HashtagSuggestion[] {
    const suggestions: HashtagSuggestion[] = []
   ...
   */
  private static generateEngagementHashtags(
    platform: Platform
  ): HashtagSuggestion[] {
    const suggestions: HashtagSuggestion[] = []
   ...
   */
  static analyzeHashtagPerformance(
    hashtags: string[],
    platform: Platform
  ): {
    score: number
    ...
    if (hashtagCount === 0) {
      feedback.push(
        "No hashtags found - adding hashtags can significantly improve reach"
      )
      score -= 30
    ...
    if (hasPlatformHashtag) {
      ...
      suggestions.push(
        `Consider adding platform hashtags like ${platformHashtags[0]}`
      )
      score -= 5
    }
    ...
    if (categories.size >= 3) {
      ...
      suggestions.push(
        "Try mixing different types of hashtags (book, genre, theme)"
      )
    }
