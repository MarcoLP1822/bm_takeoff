...
export function ContentEditor({ 
  ...
  const validationErrors = useMemo(() => {
    const errors: string[] = []
    if (isOverLimit) {
      errors.push(`Content exceeds ${config.maxLength} character limit by ${characterCount - config.maxLength} characters`)
    }
    if (hasExcessHashtags) {
      errors.push(`Too many hashtags (${hashtagArray.length}/${config.hashtagLimit})`)
    }
    return errors
  }, [isOverLimit, hasExcessHashtags, config.maxLength, characterCount, hashtagArray.length, config.hashtagLimit])
  ...
  }, [content, hashtags, hasChanges, onAutoSaveAction, autoSaveDelay, post, hashtagArray, characterCount, isValid, validationErrors])
      ...
      <div className="space-y-2">
        ...
        <Textarea
          ...
          className={cn(
            ...
            hasExcessHashtags && "border-destructive focus-visible:border-destructive"
          )}
        />
    ...
    case 'instagram':
      return (
        <div className="space-y-2">
          ...
          {imageUrl && (
            <div className="w-full h-32 bg-gray-200 rounded flex items-center justify-center text-muted-foreground text-sm">
              Image Preview
            </div>
          )}
