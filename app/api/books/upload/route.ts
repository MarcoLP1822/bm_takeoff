...
export async function POST(request: NextRequest) {
  try {
    ...
    const skipSecurityScan = isDevelopment && process.env.SKIP_SECURITY_SCAN === 'true'
  ...
  const sanitizedTitle = InputValidator.sanitizeText(file.name.replace(/\.[^/.]+$/, ''))
  ...
  const uploadResult = await RetryService.retryNetworkOperation(async () => {
  ...
  try {
    const extractionResult = await RetryService.retryFileProcessing(async () => {
  ...
  const newBook = await RetryService.retryDatabaseOperation(async () => {
    const [book] = await db.insert(books).values({
      userId,
      title: InputValidator.sanitizeText(extractionMetadata.title || sanitizedTitle),
      author: extractionMetadata.author ? InputValidator.sanitizeText(extractionMetadata.author) : null,
  ...
  const response = NextResponse.json({
    ...
    ...(extractionError && {
      warnings: [{
        ...
        message: 'Text extraction failed but file was uploaded successfully',
        retryable: true
      }]
    })
  })
