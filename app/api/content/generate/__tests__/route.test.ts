...
const mockGenerateSocialContent = contentGeneration.generateSocialContent as jest.MockedFunction<typeof contentGeneration.generateSocialContent>
...
const POST = async (request: NextRequest): Promise<NextResponse> => {
  try {
    ...
    // Mock book query
    const book = await db.select().from({} as any).where({} as any).limit(1)
    ...
    try {
      const contentVariations = await contentGeneration.generateSocialContent(
        ...
        bookData.author,
        {
          ...
          tone: ['professional', 'casual', 'inspirational', 'educational'].includes(options.tone) 
...
describe('/api/content/generate', () => {
  ...
  test('should handle rate limit errors', async () => {
    mockGenerateSocialContent.mockRejectedValue(new Error('rate limit exceeded'))
  ...
  test('should handle valid tone options', async () => {
    const validTones = ['professional', 'casual', 'inspirational', 'educational']
