...
describe("/api/content/variations GET", () => {
  ...
  it("fetches content variations successfully", async () => {
    ...
    expect(data.data.variations[0].posts[0].content).toBe(
      "Great quote from a book!"
    )
  })
  ...
  it("applies book filter when bookId is provided", async () => {
    ...
    const request = new NextRequest(
      "http://localhost/api/content/variations?bookId=book-123"
    )
    await GET(request)
  ...
  it("applies platform filter when platform is provided", async () => {
    ...
    const request = new NextRequest(
      "http://localhost/api/content/variations?platform=twitter"
    )
    await GET(request)
  ...
  it("applies status filter when status is provided", async () => {
    ...
    const request = new NextRequest(
      "http://localhost/api/content/variations?status=published"
    )
    await GET(request)
  ...
  it("applies limit and offset for pagination", async () => {
    ...
    const request = new NextRequest(
      "http://localhost/api/content/variations?limit=10&offset=20"
    )
    await GET(request)
