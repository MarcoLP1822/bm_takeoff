...
describe('InputValidator', () => {
  ...
  describe('validateString', () => {
    ...
    it('should validate maximum length', () => {
      const result = InputValidator.validateString('abcdefghijk', { maxLength: 5 })
    ...
    it('should validate pattern', () => {
      ...
      const result = InputValidator.validateString('invalid-email', { pattern: emailPattern })
    ...
    it('should use custom validator', () => {
      ...
      const result = InputValidator.validateString('no match', { customValidator })
    ...
    it('should use custom validator with custom error message', () => {
      const customValidator = (value: unknown): string | boolean => 
        typeof value === 'string' && value.includes('test') ? true : 'Must contain "test"'
      const result = InputValidator.validateString('no match', { customValidator })
  ...
  describe('validateSocialContent', () => {
    ...
    it('should reject Twitter content over limit', () => {
      ...
      expect(result.isValid).toBe(false)
      expect(result.errors).toContain('Content exceeds twitter character limit of 280')
    })
    ...
    it('should detect suspicious content', () => {
      ...
      expect(result.isValid).toBe(false)
      expect(result.errors).toContain('Content contains potentially harmful elements')
    })
  ...
  describe('validateFileMetadata', () => {
    it('should validate clean metadata', () => {
      ...
      expect(result.isValid).toBe(true)
      expect((result.sanitizedValue as Record<string, string>).title).toBe('Clean Book Title')
      expect((result.sanitizedValue as Record<string, string>).author).toBe('Clean Author Name')
    })
    ...
    it('should reject metadata with suspicious content', () => {
      ...
      expect(result.isValid).toBe(false)
      expect(result.errors).toContain('Title: Content contains potentially harmful elements')
    })
    ...
    it('should reject metadata exceeding length limits', () => {
      ...
      expect(result.isValid).toBe(false)
      expect(result.errors).toContain('Title: Must be no more than 255 characters long')
    })
  })
  ...
  describe('validateApiRequest', () => {
    it('should validate API request body', () => {
      ...
      expect(result.isValid).toBe(true)
      expect((result.sanitizedValue as Record<string, string>).title).toBe('Valid Title')
      expect((result.sanitizedValue as Record<string, string>).content).toBe('Valid content')
    })
    ...
    it('should reject invalid API request body', () => {
      ...
      expect(result.errors).toContain('content: Must be no more than 1000 characters long')
    })
  })
})