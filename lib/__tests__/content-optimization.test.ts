...
describe("Content Optimization", () => {
  describe("optimizeContent", () => {
    test("should optimize Twitter content with line breaks", () => {
      const content =
        "This is a sentence. This is another sentence. Final thought."
    ...
    test("should format LinkedIn content with bullet points", () => {
      const content =
        "Key insights: leadership matters, innovation drives success, teamwork is essential, communication is key"
  ...
  describe("suggestImprovements", () => {
    ...
    test("should suggest adding hashtags for low hashtag count", () => {
      ...
      const suggestions = suggestImprovements(post)
      expect(suggestions.some(s => s.includes("more relevant hashtags"))).toBe(
        true
      )
    })
    ...
    test("should suggest call-to-action for content without engagement", () => {
      ...
      const suggestions = suggestImprovements(post)
      expect(
        suggestions.some(s => s.includes("question or call-to-action"))
      ).toBe(true)
    })
    ...
    test("should suggest emojis for Instagram without book emojis", () => {
      ...
      const suggestions = suggestImprovements(post)
      expect(suggestions.some(s => s.includes("book-related emojis"))).toBe(
        true
      )
    })
