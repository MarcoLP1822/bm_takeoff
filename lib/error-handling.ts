...
export const ErrorCreators = {
  ...
  forbidden: (resource: string, details?: ErrorDetails) =>
    new AppError(
  ...
  // Validation errors
  validation: (field: string, message: string, details?: ErrorDetails) =>
    new AppError(
  ...
  unsupportedFileFormat: (supportedFormats: string[]) =>
    new AppError(
  ...
  textExtractionFailed: (fileName: string) =>
    new AppError(
  ...
  socialMediaApiError: (platform: string, details?: ErrorDetails) =>
    new AppError(
      `${platform} API error`,
      ErrorType.EXTERNAL_SERVICE,
      502,
  ...
  // Database errors
  databaseError: (operation: string, details?: ErrorDetails) =>
    new AppError(
  ...
  // Rate limiting
  rateLimitExceeded: (limit: number, resetTime?: Date) =>
    new AppError(
      "Rate limit exceeded",
  ...
  contentGenerationFailed: (reason?: string, details?: ErrorDetails) =>
    new AppError(
  ...
  socialAccountExpired: (platform: string) =>
    new AppError(
  ...
  // Scheduling errors
  schedulingFailed: (reason?: string, details?: ErrorDetails) =>
    new AppError(
  ...
  invalidScheduleTime: (scheduledTime: Date) =>
    new AppError(
  ...
  // Analytics errors
  analyticsUnavailable: (platform?: string) =>
    new AppError(
  ...
  bookAnalysisInProgress: (bookTitle: string) =>
    new AppError(
  ...
  bookAnalysisRequired: (bookTitle: string) =>
    new AppError(
  ...
  contentAlreadyPublished: (contentId: string) =>
    new AppError(
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        const stored = JSON.parse(
          localStorage.getItem("error_analytics") || "[]"
        )
  ...
  switch (error.type) {
    ...
    case ErrorType.FILE_PROCESSING:
      ...
      if (
        ...
        Array.isArray(error.details.supportedFormats)
      ) {
        suggestions.push(
          `Supported formats: ${(error.details.supportedFormats as string[]).join(", ")}`
        )
      }
      break
    ...
    case ErrorType.RATE_LIMIT:
      if (
        ...
        typeof error.details.resetTime === "string"
      ) {
    ...
    case ErrorType.EXTERNAL_SERVICE:
      ...
      if (error.details?.platform) {
        suggestions.push(
          `Check your ${error.details.platform} account connection`
        )
      }
      break
