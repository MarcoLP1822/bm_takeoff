...
jest.mock('@/db/schema', () => ({
  ...
  postAnalytics: { contentId: 'contentId', id: 'id', lastUpdated: 'lastUpdated' }
}))
...
describe('GDPRComplianceService', () => {
  ...
  describe('exportUserData', () => {
    ...
    it('should handle export errors gracefully', async () => {
      ...
      await expect(GDPRComplianceService.exportUserData('test-user-id'))
  ...
  describe('anonymizeUserData', () => {
    it('should anonymize user data successfully', async () => {
      ...
      const result = await GDPRComplianceService.anonymizeUserData('test-user-id')
  ...
  describe('validateConsent', () => {
    ...
    it('should reject old consent', () => {
      const oldConsent = {
        ...
        marketing: false,
        functional: true,
        timestamp: new Date(Date.now() - 2 * 365 * 24 * 60 * 60 * 1000).toISOString() // 2 years ago
      }
  ...
  describe('shouldRetainData', () => {
    ...
    it('should handle unknown data types', () => {
      ...
      const result = GDPRComplianceService.shouldRetainData(recentDate, 'unknown')
  ...
  describe('logDataProcessing', () => {
    it('should log data processing activity', async () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith('GDPR Activity Log:', expect.objectContaining(activity))
      expect(consoleSpy).toHaveBeenCalledWith('Encrypted audit log created:', expect.stringContaining('...'))
