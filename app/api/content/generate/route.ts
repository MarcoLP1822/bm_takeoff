...
import { generateSocialContent, ContentGenerationOptions } from '@/lib/content-generation'
...
const contentGenerationSchema = z.object({
  ...
  options: z.object({
    ...
    tone: z.enum(['professional', 'casual', 'inspirational', 'educational']).optional(),
...
export async function POST(request: NextRequest) {
  try {
    ...
    const bookData = await RetryService.retryDatabaseOperation(async () => {
      const book = await db
        .select()
        .from(books)
    ...
    if (!analysisData.quotes || !analysisData.keyInsights || !analysisData.themes) {
      return NextResponse.json(
    ...
    const validatedOptions: ContentGenerationOptions = {
      platforms: options.platforms || ['twitter', 'instagram', 'linkedin', 'facebook'],
      ...
      tone: options.tone && ['professional', 'casual', 'inspirational', 'educational'].includes(options.tone) 
        ? options.tone 
        : 'inspirational',
    ...
    const contentVariations = await RetryService.retryAIService(async () => {
