...
export class ToastService {
  ...
  static error(message: string, options?: {
    ...
    const action = options?.retryable && options?.onRetry ? {
      label: 'Retry',
  ...
  static loading(message: string, promise: Promise<unknown>, options?: {
    success?: string
    error?: string
    finally?: () => void
  }) {
  ...
  static handleError(error: AppError | Error, options?: {
    onRetry?: () => void
    context?: string
  }) {
      ...
      switch (error.type) {
        case ErrorType.VALIDATION:
          return this.warning(error.userMessage, {
            description: error.details?.field ? `Field: ${error.details.field}` : undefined,
            duration: 5000
          })
        ...
        case ErrorType.RATE_LIMIT:
          ...
          const resetMessage = resetTime && (typeof resetTime === 'string' || typeof resetTime === 'number' || resetTime instanceof Date)
  ...
  static analysis = {
    start: (bookTitle: string) => 
      toast.loading(`Analyzing "${bookTitle}"...`),
  ...
  static publishing = {
    start: (platform: string, accountCount: number) => 
      toast.loading(`Publishing to ${accountCount} ${platform} account${accountCount > 1 ? 's' : ''}...`),
    
    success: (successCount: number, totalCount: number) => {
      if (successCount === totalCount) {
        return this.success(`Published to ${successCount} account${successCount > 1 ? 's' : ''}`)
      } else {
    ...
    error: (failCount: number, totalCount: number, onRetry?: () => void) => 
      this.error(`Failed to publish to ${failCount} account${failCount > 1 ? 's' : ''}`, {
        description: totalCount > failCount ? `${totalCount - failCount} succeeded` : undefined,
        retryable: !!onRetry,
        onRetry,
        duration: 7000
      })
  ...
  static scheduling = {
    ...
    cancelled: () => 
      this.success('Scheduled post cancelled'),
  ...
  static contentGeneration = {
    start: (bookTitle: string, platforms: string[]) =>
      toast.loading(`Generating content for "${bookTitle}" (${platforms.join(', ')})...`),
    ...
    partialSuccess: (successCount: number, totalCount: number, platforms: string[]) =>
  ...
  static analytics = {
    updated: (platform?: string) =>
      this.success('Analytics updated', {
        description: platform ? `${platform} data refreshed` : 'All platform data refreshed'
      }),
